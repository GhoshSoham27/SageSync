    ...
    return <nav className="sticky z-[100] h-16 inset-x-0 top-0 w-full border-b border-gray-200 bg-white/80 backdrop-blur-lg transition-all">
        <MaxWidthWrapper>
            <div className="flex h-16 items-center justify-between">
                <Link href="/" className="flex z-40 font-semibold">Sage<span className="text-brand-700">Sync</span></Link>
                <div className="h-full flex items-center space-x-4">
                    {user ? <>
                       <SignOutButton>
                            <Button size="sm" variant="ghost">Sign out</Button>
                        </SignOutButton> 
                        <Link href='/dashboard' className={buttonVariants({size: "sm", className: "flex items-center gap-1"})}>
                        ...
                        <>
                            ...
                            <Link href='/sign-up' className={buttonVariants({size: "sm", className: "flex items-center gap-1.5" })}>
